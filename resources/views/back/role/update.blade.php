...
<div class="container-fluid flex-grow-1 container-p-y">
    ...
    <div class="card mb-4">
        ...
        <form class="card-body" method="POST" action="{{route('role_update', ['role' => $record->id])}}" id="formValidationUser">
            @csrf
            <div class="row g-3">
                <div class="col-md-6">
                    <label class="form-label" for="role">Role</label>
                    <input type="text" name="role" id="role" class="form-control" placeholder="Role" value="{{old('role', $record->role)}}" />
                    @error('role')
                ...
                <div class="col-md-6">
                    ...
                    <select id="select2Basic" class="select2 form-select" name="color" data-allow-clear="true">
                        ...
                        <option value="primary" class="text-white bg-primary" {{$record->color == "primary" ? 'selected' : ''}}>Bleu</option>
                        <option value="info" class="text-white bg-info" {{$record->color == "info" ? 'selected' : ''}}>Bleu Ciel</option>
                        <option value="danger" class="text-white bg-danger" {{$record->color == "danger" ? 'selected' : ''}}>Rouge</option>
                        <option value="success" class="text-white bg-success" {{$record->color == "success" ? 'selected' : ''}}>Vert</option>
                        <option value="warning" class="text-white bg-warning" {{$record->color == "warning" ? 'selected' : ''}}>Jaune</option>
                        <option value="secondary" class="text-white bg-secondary" {{$record->color == "secondary" ? 'selected' : ''}}>Gris</option>
                        <option value="light" class="text-dark bg-light" {{$record->color == "light" ? 'selected' : ''}}>Blanc</option>
                        <option value="dark" class="text-white bg-dark" {{$record->color == "dark" ? 'selected' : ''}}>Noir</option>
                    </select>
                    @error('color')
                    <span class="helper-text text-danger">{{ $message }}</span>
                    @enderror
                ...
                <div class="col-md-12">
                    ...
                    <textarea name="desc" id="desc" class="form-control" placeholder="Description">{{old('desc', $record->desc)}}</textarea>
                    @error('desc')
                    <span class="helper-text text-danger">{{ $message }}</span>
                    @enderror
                </div>
                                ...
                                <div class="col-md-12">
                                    <div class="mb-3">
                                        <select multiple="multiple" size="10" name="fonctionnalites[]" id="selectpickerBasic" class="selectpicker w-100" data-style="btn-default">
                                            ...
                                            <option {{ in_array($fonctionnalite->id, $record->fonctionnalites()->allRelatedIds()->toArray()) ? "selected" : '' }} value="{{ $fonctionnalite->id }}">{{ $fonctionnalite->name }}
                                            </option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <div class="col-md-12">
                                    <div class="mb-3">
                                        <select multiple="multiple" size="10" name="permissions[]" id="selectpickerBasic" class="selectpicker w-100" data-style="btn-default">
                                            ...
                                            <option {{ in_array($permission->id, $record->permissions()->allRelatedIds()->toArray()) ? "selected" : '' }} value="{{ $permission->id }}">{{ $permission->name }}
                                            </option>
                                            @endforeach
                                        </select>
                                    </div>
