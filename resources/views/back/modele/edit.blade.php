...
<div class="container-fluid flex-grow-1 container-p-y">
    ...
    <div class="card mb-4">
        ...
        <form class="card-body" method="POST" action="{{ route('modele.update', ['modele' => $record->id]) }}">
            @csrf
            @method('Put')
            <div class="row g-3">
                   @can('modele.marque_id.update')
   <div class="col-md-6">
    ...
    <option class="option" {{ $row->id == old('marque_id', $record->marque_id) ? 'selected' : '' }} value="{{ $row->id }}"> {{ $row->title }}</option>
    @endforeach
</select>
       @error('marque_id')
   ...
   @endcan   @can('modele.title.update')
   <div class="col-md-6">
   ...
   @endcan   @can('modele.value.update')
   <div class="col-md-6">
