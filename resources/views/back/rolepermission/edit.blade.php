...
<div class="container-fluid flex-grow-1 container-p-y">
    ...
    <div class="card mb-4">
        ...
        <form class="card-body" method="POST" action="{{ route('rolepermission.update', ['rolepermission' => $record->id]) }}">
            @csrf
            @method('Put')
            <div class="row g-3">
                    <div class="col-md-6">
    ...
    <option class="option" {{ $row->id == old('role_id', $record->role_id) ? 'selected' : '' }} value="{{ $row->id }}"> {{ $row->label }}</option>
    @endforeach
</select>
        @error('role_id')
    ...
    <option class="option" {{ $row->id == old('permission_id', $record->permission_id) ? 'selected' : '' }} value="{{ $row->id }}"> {{ $row->name }}</option>
    @endforeach
</select>
        @error('permission_id')
        <span class="helper-text text-danger">{{ $message }}</span>
