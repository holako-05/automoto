...
@section('content')
    <div class="container-fluid flex-grow-1 container-p-y">
        ...
        <div class="card mb-4">
            ...
            <form class="card-body" method="POST" action="{{ route('statut.update', ['statut' => $record->id]) }}">
                @csrf
                @method('Put')
                <div class="row g-3">
                    @can('statut.code.update')
                        <div class="col-md-6">
                    ...
                    @can('statut.color.update')
                        <div class="col-md-6">
                            ...
                            <select id="select2Basic" class="form-select" name="color" data-allow-clear="true">
                                ...
                                <option value="primary" class="text-white bg-primary"
                                    {{ $record->color == 'primary' ? 'selected' : '' }}>Bleu</option>
                                <option value="info" class="text-white bg-info"
                                    {{ $record->color == 'info' ? 'selected' : '' }}>Bleu Ciel</option>
                                <option value="danger" class="text-white bg-danger"
                                    {{ $record->color == 'danger' ? 'selected' : '' }}>Rouge</option>
                                <option value="success" class="text-white bg-success"
                                    {{ $record->color == 'success' ? 'selected' : '' }}>Vert</option>
                                <option value="warning" class="text-white bg-warning"
                                    {{ $record->color == 'warning' ? 'selected' : '' }}>Jaune</option>
                                <option value="secondary" class="text-white bg-secondary"
                                    {{ $record->color == 'secondary' ? 'selected' : '' }}>Gris</option>
                                <option value="light" class="text-dark bg-light"
                                    {{ $record->color == 'light' ? 'selected' : '' }}>Blanc</option>
                                <option value="dark" class="text-white bg-dark"
                                    {{ $record->color == 'dark' ? 'selected' : '' }}>Noir</option>
                            </select>
                            @error('color')
                                <span class="helper-text text-danger">{{ $message }}</span>
                            @enderror
