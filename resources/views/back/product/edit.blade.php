...
@section('content')
    <div class="container-fluid flex-grow-1 container-p-y">
        ...
        <div class="card mb-4">
            ...
            <form class="card-body" method="POST" action="{{ route('product.update', ['product' => $record->id]) }}">
                @csrf
                @method('Put')
                <div class="row g-3">
                    @can('product.category_id.update')
                        <div class="col-md-4">
                        ...
                        @endcan @can('product.name.update')
                        <div class="col-md-4">
                        ...
                        @endcan @can('product.stock_quantity.update')
                        <div class="col-md-4">
                            ...
                            <input type="text" name="stock_quantity" id="stock_quantity" class="form-control"
                                placeholder="Quantité en stock" value="{{ old('stock_quantity', $record->stock_quantity) }}" />
                            @error('stock_quantity')
                                <span class="helper-text text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        @endcan @can('product.type.update')
                        <div class="col-md-4">
                            <label class="form-label" for="type">Type</label>
                            <select name="type" id="type" class="select2 form-control" data-allow-clear="true">
                                <option value="stockable" {{ $record->type == 'stockable' ? 'selected' : '' }}> Stockable
                                </option>
                                "<option value="nstockable" {{ $record->type == 'nstockable' ? 'selected' : '' }}>
                                    Non-Stockable </option>
                                "
                            </select>
                            @error('type')
                                <span class="helper-text text-danger">{{ $message }}</span>
