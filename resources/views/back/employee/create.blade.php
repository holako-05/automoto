...
@section('content')
    <div class="container-fluid flex-grow-1 container-p-y">
        ...
        <div class="card mb-4">
            ...
            <form class="card-body" method="POST" action="{{ route('employee.store') }}">
                @csrf
                <div class="row g-3">
                    @can('employee.last_name.create')
                        <div class="col-md-4">
                    ...
                    @endcan
                    @can('employee.first_name.create')
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="user_id" id="user_id" class="select2 form-control" data-allow-clear="true">
                                @foreach ($userRecords as $row)
                                    <option class="option" {{ $row->id == old('user_id') ? 'selected' : '' }}
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="position" id="position" class="select2 form-control" data-allow-clear="true">
                                @foreach ($positionRecords as $row)
                                    <option class="option" {{ $row->id == old('position') ? 'selected' : '' }}
                    ...
                    @endcan
                    @can('employee.speciality.create')
                        <div class="col-md-4">
                            ...
                            <select name="speciality" id="speciality" class="select2 form-control" data-allow-clear="true">
                                @foreach ($specialityRecords as $row)
                                    <option class="option" {{ $row->id == old('speciality') ? 'selected' : '' }}
                                        value="{{ $row->id }}"> {{ $row->label }}</option>
                                @endforeach
                            </select>
                            @error('speciality')
                    ...
                    @can('employee.hire_date.create')
                        ...
                        @endcan @can('employee.shift_start_time.create')
                        <div class="col-md-4">
                            ...
                            <input type="text" class="form-control quicky-time" placeholder="HH:MM" id="shift_start_time"
                                name="shift_start_time" value="{{ old('shift_start_time') }}" />
                            @error('shift_start_time')
                                <span class="helper-text text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        @endcan @can('employee.shift_end_time.create')
                        <div class="col-md-4">
                            ...
                            <input type="text" class="form-control quicky-time" placeholder="HH:MM" id="shift_end_time"
                                name="shift_end_time" value="{{ old('shift_end_time') }}" />
                            @error('shift_end_time')
                                <span class="helper-text text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        @endcan @can('employee.salary.create')
                        <div class="col-md-4">
                    ...
                    @endcan
                    @can('employee.emergency_contact.create')
                        <div class="col-md-4">
                            ...
                            <input type="text" name="emergency_contact" id="emergency_contact" class="form-control"
                                placeholder="Contact d'urgence" value="{{ old('emergency_contact') }}" />
                            @error('emergency_contact')
                                <span class="helper-text text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                    @endcan
                    @can('employee.address.create')
                        <div class="col-md-4">
