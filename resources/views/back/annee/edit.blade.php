...
<div class="container-fluid flex-grow-1 container-p-y">
    ...
    <div class="card mb-4">
        ...
        <form class="card-body" method="POST" action="{{ route('annee.update', ['annee' => $record->id]) }}">
            @csrf
            @method('Put')
            <div class="row g-3">
                   @can('annee.year.update')
   <div class="col-md-6">
       <label class="form-label" for="year">Année</label>
       <input type="text" name="year" id="year" class="form-control" placeholder="Année" value="{{ old('year', $record->year) }}" />
       @error('year')
