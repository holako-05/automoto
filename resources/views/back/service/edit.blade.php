...
<div class="container-fluid flex-grow-1 container-p-y">
    ...
    <div class="card mb-4">
        ...
        <form class="card-body" method="POST" action="{{ route('service.update', ['service' => $record->id]) }}">
            @csrf
            @method('Put')
            <div class="row g-3">
                   @can('service.title.update')
   <div class="col-md-6">
       <label class="form-label" for="title">Nom de Service</label>
       <input type="text" name="title" id="title" class="form-control" placeholder="Nom de Service" value="{{ old('title', $record->title) }}" />
       @error('title')
       <span class="helper-text text-danger">{{ $message }}</span>
       @enderror
   </div>
   @endcan   @can('service.type_service_id.update')
   <div class="col-md-6">
    ...
    <option class="option" {{ $row->id == old('type_service_id', $record->type_service_id) ? 'selected' : '' }} value="{{ $row->id }}"> {{ $row->title }}</option>
    @endforeach
</select>
