...
<div class="container-fluid flex-grow-1 container-p-y">
    ...
    <div class="card mb-4">
        ...
        <form class="card-body" method="POST" action="{{ route('facture_article.store') }}">
            @csrf
            <div class="row g-3">
                   @can('facture_article.article.create')
   <div class="col-md-6">
       <label class="form-label" for="article">Article</label>
       <input type="text" name="article" id="article" class="form-control" placeholder="Article" value="{{ old('article') }}" />
       @error('article')
       <span class="helper-text text-danger">{{ $message }}</span>
       @enderror
   </div>
   @endcan   @can('facture_article.facture_id.create')
   <div class="col-md-6">
    ...
    <option class="option" {{ $row->id == old('facture_id') ? 'selected' : '' }} value="{{ $row->id }}"> {{ $row->code }}</option>
    @endforeach
</select>
       @error('facture_id')
       <span class="helper-text text-danger">{{ $message }}</span>
